...
func obtenerHora (time: String) -> String {
    
    var meridiem = "AM"
    var tiempoVisualizar = 1
    let TiempoMilitar = time.suffix(5)
    let tiempoActual = TiempoMilitar.prefix(2)
    if (tiempoActual == "00" || tiempoActual == "12") {
        tiempoVisualizar = 12
        if(tiempoActual == "00") {
            meridiem = "AM"
        } else {
        ...
        if let enTiempo = Int(tiempoActual) {
            if(enTiempo >= 13) {
                tiempoVisualizar = enTiempo - 12
                meridiem = "PM"
            } else {
                tiempoVisualizar = enTiempo
                meridiem = "AM"
            }
        }
    ...
    return "\(tiempoVisualizar)\(meridiem)"
}
